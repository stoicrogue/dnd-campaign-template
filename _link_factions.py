"""
Link Factions in Campaign Notes
================================
Scans markdown files and converts unlinked faction name references
into Obsidian wiki-links ([[...]]).

Usage:
    python _link_factions.py          # apply changes
    python _link_factions.py --dry-run  # preview changes without writing
"""
...
# ---------------------------------------------------------------------------
# Faction definitions: (search_term, link_target, display_text_or_None)
#
...
#
# If a faction has multiple spellings (e.g. curly vs straight apostrophe),
# list the canonical spelling first, then the variant with a display override:
#   ("Faction\u2019s Name",  "Faction\u2019s Name",  None),
#   ("Faction's Name",       "Faction\u2019s Name",  "Faction's Name"),
# ---------------------------------------------------------------------------
FACTIONS = [
    # Add your factions here:
    # ("Faction Name",  "Faction Name",  None),
]
# ===========================================================================
...
def process_line(line, in_frontmatter, in_code_block, factions=None):
    ...
    for search_term, target, display in (factions if factions is not None else FACTIONS):
...
def process_file(filepath, dry_run=False):
    ...
    # Skip factions whose link target matches this file (no self-links)
    file_stem = os.path.splitext(os.path.basename(filepath))[0]
    factions = [(s, t, d) for s, t, d in FACTIONS if t != file_stem]
    ...
    for line in lines:
        new_line, in_frontmatter, in_code_block = process_line(line, in_frontmatter, in_code_block, factions)
